...
class MediaDetector {
  MediaType detectType(String directoryPath, List<String> files) {
    ...
    // Check filename for year pattern (like Movie.2010.mkv) - indicates movies
    final hasYearInFilename = files.any((file) =>
        RegExp(r'\b(19|20)\d{2}\b').hasMatch(file));

    if (hasYearInFilename) {
      return MediaType.movie;
    }

    // Check if this is a subdirectory with video files (typical movie structure)
    ...
    if (directoryPath.isNotEmpty && directoryPath != '.' && videoFiles.isNotEmpty) {
      return MediaType.movie;
    }
